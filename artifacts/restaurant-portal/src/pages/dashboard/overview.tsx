...
import { Loader2, TrendingUp, Users, Clock, CheckCircle2, AlertTriangle, AlertCircle } from "lucide-react";
...
export default function Overview() {
  ...
  if (!data) return <div className="text-muted-foreground p-4">Failed to load overview data</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-card">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{inr(data.pendingSettlementAmount)}</div>
        ...
        <Card className="bg-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Lunch: {data.lunchLockedCount} &bull; Dinner: {data.dinnerLockedCount}
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="lg:col-span-4 bg-card">
          <CardHeader>
            <CardTitle>Cancellation Impact (Last 30 Days)</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-3 gap-4">
            <div className="flex flex-col items-center justify-center p-4 bg-muted/50 rounded-lg">
              ...
              <span className="text-xs text-muted-foreground mt-1">No deduction</span>
            </div>
            <div className="flex flex-col items-center justify-center p-4 bg-orange-50 border border-orange-200 rounded-lg">
              <span className="text-sm text-orange-700 mb-1 flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" /> Late
              </span>
              <span className="text-xl font-bold text-orange-700">{data.lateCancellations}</span>
              <span className="text-xs text-orange-600 mt-1">50% deducted</span>
            </div>
            <div className="flex flex-col items-center justify-center p-4 bg-red-50 border border-red-200 rounded-lg">
              <span className="text-sm text-red-700 mb-1 flex items-center gap-1">
                <AlertCircle className="w-3 h-3" /> No Shows
              </span>
              <span className="text-xl font-bold text-red-700">{data.noShows}</span>
              <span className="text-xs text-red-600 mt-1">Full payout kept</span>
            </div>
          </CardContent>
        </Card>
                ...
                data.recentActivity.map((activity, i) => (
                  <div key={i} className="flex items-start gap-4">
                    ...
                    <div className="flex-1 space-y-1">
                      <p className="text-sm font-medium leading-none capitalize">{activity.description}</p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(activity.timestamp).toLocaleString("en-IN")}
                        {activity.amount ? ` • ${inr(activity.amount)}` : ""}
                      </p>
                    </div>
                  </div>
                ))
