...
export default function HomeScreen() {
  ...
  const QUICK_ACTIONS = [
    { icon: "plus-circle" as const, label: "Subscribe", color: "#3B82F6", bg: "#EFF6FF", onPress: () => router.push("/(tabs)/meals") },
    { icon: "credit-card" as const, label: "My Plans", color: "#8B5CF6", bg: "#EDE9FE", onPress: () => router.push("/(tabs)/pass") },
    { icon: "list" as const, label: "Orders", color: "#16A34A", bg: "#DCFCE7", onPress: () => router.push("/(tabs)/orders") },
    { icon: "help-circle" as const, label: "Support", color: "#3B82F6", bg: "#DBEAFE", onPress: () => router.push("/support") },
  ];
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}> 
        ...
        <LinearGradient colors={["#3B82F6", "#2563EB"]} style={[styles.header, { paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) + 16 }]}>
          ...
          {activeSubs.length > 0 && (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.subPillsScroll} contentContainerStyle={styles.subPillsContent}>
              {activeSubs.map((sub) => (
                <Pressable key={sub.id} onPress={() => router.push("/(tabs)/pass")} style={styles.subPill}>
                  <Text style={styles.subPillName}>{sub.restaurantName}</Text>
                  <Text style={styles.subPillDetail}>{sub.slot === "lunch" ? "☀️" : "🌙"} {sub.remainingDays} days left</Text>
                </Pressable>
              ))}
            </ScrollView>
          )}
        ...
        <View style={styles.content}>
          {todaysMeals.length > 0 && (
            <View style={styles.section}>
              ...
              {todaysMeals.map((order) => <MealOrderCard key={order.id} order={order} onCancel={setCancelTarget} />)}
            </View>
          )}

          <View style={styles.quickActions}>
            {QUICK_ACTIONS.map((action) => (
              <Pressable key={action.label} onPress={action.onPress} style={({ pressed }) => [styles.quickAction, pressed && { opacity: 0.8 }]}>
                <View style={[styles.quickActionIcon, { backgroundColor: action.bg }]}><Feather name={action.icon} size={20} color={action.color} /></View>
          ...
          {upcomingOrders.length > 0 && (
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                ...
                <Pressable onPress={() => router.push("/(tabs)/orders")}><Text style={[styles.seeAll, { color: colors.primary }]}>See all</Text></Pressable>
              </View>
          ...
          {activeSubs.length === 0 && (
            <View style={[styles.emptyState, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <Text style={[styles.emptyBody, { color: colors.mutedForeground }]}>Pick a restaurant, choose lunch or dinner, pick a 10, 20, or 30-day pack — and every meal is auto-scheduled.</Text>
              <Pressable onPress={() => router.push("/(tabs)/meals")} style={styles.discoverBtn}><Text style={styles.discoverBtnText}>Discover restaurants</Text></Pressable>
            </View>
          )}
      ...
      <Modal visible={!!cancelTarget} transparent animationType="slide" onRequestClose={closeCancelSheet}>
        <Pressable style={styles.overlay} onPress={closeCancelSheet}>
          <Pressable style={styles.sheet}>
            {cancelResult ? (
              <>
                <View style={styles.sheetHandle} />
                <Text style={styles.sheetTitle}>{cancelResult.type === "free" ? "Day cancelled" : cancelResult.type === "late" ? "Late cancellation" : "Meal charged"}</Text>
                <Text style={styles.sheetBody}>{cancelResult.type === "free" ? "This day has been cancelled. No charge applied." : cancelResult.type === "late" ? `Late cancellation fee: ₹${cancelResult.fee} (50% of day rate).` : `Full charge: ₹${cancelResult.fee}. Preparation had already started.`}</Text>
                <Pressable style={styles.doneBtn} onPress={closeCancelSheet}><Text style={styles.doneBtnText}>Done</Text></Pressable>
              </>
            ) : (
              <>
                <View style={styles.sheetHandle} />
                <Text style={styles.sheetTitle}>Cancel {cancelTarget?.slot} on {cancelTarget ? formatDate(cancelTarget.scheduledDate) : ""}?</Text>
                <Text style={styles.sheetMealName}>{cancelTarget?.restaurantName} · {cancelTarget?.mealName}</Text>
                {cancelStatus === "free" && <View style={styles.freeInfo}><Feather name="check-circle" size={16} color="#16A34A" /><Text style={styles.freeInfoText}>Free cancellation available.{"\n"}No charge for this day.</Text></View>}
                {cancelStatus === "late" && <View style={styles.lateInfo}><Feather name="alert-triangle" size={16} color="#F59E0B" /><Text style={styles.lateInfoText}>Late cancellation fee applies.{"\n"}₹{cancelTarget ? Math.round(cancelTarget.pricePerDay * 0.5) : 0} (50% of day rate) will be charged.</Text></View>}
                {cancelStatus === "full" && <View style={styles.fullInfo}><Feather name="alert-circle" size={16} color="#EF4444" /><Text style={styles.fullInfoText}>Preparation has started.{"\n"}Full day charge ₹{cancelTarget?.pricePerDay} applies.</Text></View>}
                <View style={styles.sheetActions}>
                  <Pressable style={[styles.keepBtn, { borderColor: colors.border }]} onPress={closeCancelSheet}><Text style={[styles.keepBtnText, { color: colors.foreground }]}>Keep</Text></Pressable>
                  <Pressable style={[styles.cancelConfirmBtn, cancelStatus === "free" && { backgroundColor: "#16A34A" }, cancelStatus === "late" && { backgroundColor: "#F59E0B" }, cancelStatus === "full" && { backgroundColor: "#EF4444" }]} onPress={confirmCancel} disabled={cancelLoading}><Text style={styles.cancelConfirmText}>{cancelStatus === "free" ? "Cancel day" : cancelStatus === "late" ? "Cancel with fee" : "Cancel anyway"}</Text></Pressable>
                </View>
              </>
            )}
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  greeting: { fontSize: 14, fontFamily: "Inter_400Regular", color: "rgba(255,255,255,0.85)", marginBottom: 2 },
  subLine: { fontSize: 18, fontFamily: "Inter_700Bold", color: "#FFFFFF" },
  ...
  subPillsScroll: { marginTop: 14 },
  subPillsContent: { gap: 8, paddingRight: 4 },
  ...
  seeAll: { fontSize: 14, fontFamily: "Inter_500Medium", marginBottom: 10 },
  ...
  emptyState: { borderRadius: 16, borderWidth: 1, padding: 28, alignItems: "center", gap: 8 },
  ...
  discoverBtn: { marginTop: 8, paddingHorizontal: 20, paddingVertical: 10, backgroundColor: "#3B82F6", borderRadius: 10 },
  discoverBtnText: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: "#FFFFFF" },
  ...
  sheetMealName: { fontSize: 14, fontFamily: "Inter_400Regular", color: "#71717A", marginBottom: 18 },
